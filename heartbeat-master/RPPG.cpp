...
void RPPG::detectFace(Mat &frameRGB, Mat &frameGray) {
    ...
    switch (faceDetAlg) {
      case haar:
        ...
        haarClassifier.detectMultiScale(frameGray, roies, 1.1, 2, CASCADE_SCALE_IMAGE, minFaceSize); 
      break;
      case deep:
...
void RPPG::detectCorners(Mat &frameGray) {
    ...
    Point points[boxes.size()][4];
    
for (int i = 0; i < boxes.size(); i++){    
    // Define tracking region
    Mat trackingRegion = Mat::zeros(frameGray.rows, frameGray.cols, CV_8UC1);
    
    points[i][0] = Point(boxes[i].tl().x + 0.22 * boxes[i].width,
                         boxes[i].tl().y + 0.21 * boxes[i].height);
    points[i][1] = Point(boxes[i].tl().x + 0.78 * boxes[i].width,
                         boxes[i].tl().y + 0.21 * boxes[i].height);
    points[i][2] = Point(boxes[i].tl().x + 0.70 * boxes[i].width,
                         boxes[i].tl().y + 0.65 * boxes[i].height);
    points[i][3] = Point(boxes[i].tl().x + 0.30 * boxes[i].width,
                         boxes[i].tl().y + 0.65 * boxes[i].height);
    const Point *pts[1] = {points[i]};
    int npts[] = {4};
...
for (int i = 0; i < boxes.size(); i++){
    ...
    if (corners_1v.size() >= MIN_CORNERS) {
        ...
        if (transform.total() > 0) {
            // Update box
            Contour2f boxCoords;
            boxCoords.push_back(boxes[i].tl());
            boxCoords.push_back(boxes[i].br());
            Contour2f transformedBoxCoords;

            cv::transform(boxCoords, transformedBoxCoords, transform);
            boxes[i] = Rect(transformedBoxCoords[0], transformedBoxCoords[1]);

            // Update roi
            Contour2f roiCoords;
            roiCoords.push_back(roies[i].tl());
            roiCoords.push_back(roies[i].br());
            Contour2f transformedRoiCoords;
            cv::transform(roiCoords, transformedRoiCoords, transform);
            roies[i] = Rect(transformedRoiCoords[0], transformedRoiCoords[1]);
            updateMask(frameGray);
        }
    } else {
...
void RPPG::updateROI() {
   ...
   for(int i = 0; i < roies.size(); i++) {
   roies[i] = Rect(Point(boxes[i].tl().x + 0.3 * boxes[i].width, boxes[i].tl().y + 0.1 * boxes[i].height),
                     Point(boxes[i].tl().x + 0.7 * boxes[i].width, boxes[i].tl().y + 0.25 * boxes[i].height));
   update.push_back(roi);
   }
   /*
...
for(int i = 0; i < update.size(); i++) {
    ...
    rectangle(mask, roies[i], WHITE, FILLED);
}
}
...
for (k = 0; k < boxes.size(); k++){
    // Draw roi;
    rectangle(frameRGB, roies[k], GREEN);

    // Draw bounding box
    rectangle(frameRGB, boxes[k], RED);
    ...
    if (!s_f.empty() && !powerSpectrum.empty()) {
        ...
        double drawAreaTlX = boxes[k].tl().x + boxes[k].width + 20;
        double drawAreaTlY = boxes[k].tl().y;
        ...
        drawAreaTlX = boxes[k].tl().x + boxes[k].width + 20;
        drawAreaTlY = boxes[k].tl().y + box.height/2.0;
    ...
    if (faceValid) {
        ss.precision(3);
        ss << meanBpm << " bpm";
        putText(frameRGB, ss.str(), Point(boxes[k].tl().x, boxes[k].tl().y - 10), FONT_HERSHEY_PLAIN, 2, RED, 2);
    }

    // Draw FPS text
    ss.str("");
    ss << fps << " fps";
    putText(frameRGB, ss.str(), Point(boxes[k].tl().x, boxes[k].br().y + 40), FONT_HERSHEY_PLAIN, 2, GREEN, 2);
